...
def dfs(lis0, xi, yi, lis1, wid, hig, si):  # 深度优先搜索

    lis1[xi][yi] = 1                        # 标记为已搜索

    for delta in ([1, 0], [0, 1], [-1, 0], [0, -1]):
        if 0 <= xi+delta[0] < wid and 0 <= yi+delta[1] < hig:                                # 判断未越界
            if (xi % si + 1) == si and (yi % si + 1) == si:                                  # 消除切片分割单块造成的误差
                return -2
            if (xi % si + 1) == si or (yi % si + 1) == si:
                return 0
            if lis1[xi+delta[0]][yi+delta[1]] == 0 and lis0[xi+delta[0]][yi+delta[1]] == 1:  # 判断为目标 & 未标记
                dfs(lis0, xi+delta[0], yi+delta[1], lis1, wid, hig, si)                      # 进入满足条件的像素
        ...
        for xa in range(wi):                        # 对每个像素进行判断&搜索&计数
            for ya in range(hi):
                if li0[xa][ya] == 1 and li1[xa][ya] == 0:
                    xb, yb = xa, ya
                    count += dfs(li0, xb, yb, li1, wi, hi, s)
...
def main(fpp, tr=250):          # 主程序函数，检测并阻止栈溢出
    try:
        ...
        ims = ims.resize((int(w/(2**0.5)), int(h/(2**0.5))), '''Im.ANTIALIAS''')   # 被注释部分为抗锯齿选项 会导致个数减小
        ims.save(fpp + 's.jpg')                                                    # 是否是因为处理了毛边？
